...
const webViewHeight = Dimensions.get('window').height + 10000
...
export default class ModalComponent extends PureComponent {
    ...
    _handleShare() {
        ...
        message =  `${title}\n\nLeia mais @\n${url}\n\ncompartilhado via Portal de Noticias`
        return Share.share(
        ...
        if (url !== undefined) {
            return (
                <Modal onRequestClose={this._handleClose} visible={showModal} transparent animationType="slide">
                    <Container style={{margin:0, marginBottom:0,backgroundColor:'#ffffff'}}>
                        <Header>
                            <Left>
